...
- 在高压力阶段里程碑处使用同一 DeepSeek 模型生成结构化语义摘要，不影响主任务失败处理；
- 工具原文按 SHA-256 保存在本地，可用 `read_tool_result` 精确分页回查被省略区间；
...
每个会话固定工作区、模式和最大步数；需要切换项目时请新建会话。若工作区根目录存在 `AGENTS.md` 或 `PROJECT_RULES.md`，新会话会把它们作为常驻项目规则载入。默认上下文窗口为 64,000 Token，其中预留 8,000 Token 给模型回答并保留 3% 安全余量。低于输入预算 70% 时保留原文；70% 起确定性压缩旧工具结果；82% 起压缩旧 exchange，并在完成步骤或测试等里程碑后生成固定字段的语义摘要；92% 起只保留固定层、检查点、当前任务和最近两个完整工具轮次。所有裁剪均保持 tool call 与 result 配对。
...
| `read_tool_result` | 分页回查被压缩的完整工具结果 | 仅接受压缩元数据返回的 SHA-256 结果 ID |
