...
- 分层上下文：固定安全规则、`AGENTS.md`/`PROJECT_RULES.md`、任务摘要、当前 Todo 与近期完整轨迹；
- 持久化 Trace：模型请求、工具参数/结果、耗时、错误码、Token 用量和最后一次测试结果；
...
后续轮次会收到同一会话中的用户消息、Agent 最终回答和近期工具轨迹。会话历史保存在本地 `.coding-agent/sessions/`，运行 Trace 保存在 `.coding-agent/traces/`。两者均被 Git 忽略，同时文件工具不允许 Agent 读写。

每个会话固定工作区、模式和最大步数；需要切换项目时请新建会话。若工作区根目录存在 `AGENTS.md` 或 `PROJECT_RULES.md`，新会话会把它们作为常驻项目规则载入。默认上下文预算为 120,000 字符，超出后会按完整用户轮次压缩，不会拆散 tool call 与对应结果。
