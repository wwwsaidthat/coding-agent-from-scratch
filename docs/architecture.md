...
超预算时先移除最旧的完整 exchange，并保留用户目标与最终结果摘要。若单个 exchange 内部仍然过长，再整体移除最早的 assistant tool calls 及其所有 tool results，至少优先保留最近两个完整工具轮次。任何裁剪都不会拆散 tool call 与对应 result。
...
超过 16,000 字符的工具结果在进入模型上下文前会改成首尾预览，并附带原始长度、返回长度、SHA-256 与压缩策略。未经压缩的结果仍写入权限为 `0600` 的本地 Trace；浏览器展示层另设上限，避免页面一次渲染过大的日志。
...
模型请求和工具事件使用同一递增序号。浏览器只接收渲染所需的精简事件；本地 Trace 额外保留去除隐藏推理字段后的请求消息、工具 schema 和完整工具结果。测试命令会被识别并保存最后一次退出码与成功状态，可作为本次运行的最终评测结果。
